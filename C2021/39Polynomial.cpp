...
class myTerm {
private:
  int coeff;   // interger coefficient
...
public:
  ...
  // member functions
  myTerm ddx() const;   // derivative of a term2
...
public:
  ...
  myPolynomial(int nTerms, intn mono[]);
  //copy constructor
  ...
  int getDegree() const;
  unsigned getNumberTerms() const;
  myPolynomial ddx() const;

  // friend operators and functions
  friend bool operator== (const myPolynomial &p1, const myPolynomial& poly);
...
myPolynomial& myPolynomial::operator += (const myPolynomial& poly){
  ...
  for(termCIt it = poly.terms.begin(); it != poly.terms.end(); i++)
    terms.push_back(*it);
...
unsigned myPolynomial::getNumberTerms() const{
  // fill in
}
...
void testSimpleCase(){
  ...
  cout << "the degree pf p8(x) is " << p8.getDegree() < " and the number of terms is " << p8.getNumberTerms() << endl;
...
void testDataFromFile(int numTerms){
  ...
  cout << " and the number of terms of p2(x) is " << p2.getNumTerms()
}
